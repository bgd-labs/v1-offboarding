...
contract LendingPool is ReentrancyGuard, VersionedInitializable {
  ...
  function rebalanceStableBorrowRate(
    ...
    if (
      userCurrentStableRate < liquidityRate || userCurrentStableRate > rebalanceDownRateThreshold
    ) {
...
contract DoubleSlopeInterestRateStrategyBase is IReserveInterestRateStrategy {
    ...
    )
  {
    ...
    if (utilizationRate > _optimalRatio) {
      uint256 excessUtilizationRateRatio = utilizationRate.sub(_optimalRatio).rayDiv(excessRatio);
